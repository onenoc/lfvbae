...
def iterate(params,num_samples,num_particles,i,m,v):
    ...
    b_2 = 0.999
    e = 10e-8
    LB = lower_bound(params,num_samples,num_particles)
    g = -grad_KL(params, num_samples,num_particles,LB)
    m = b_1*m+(1-b_1)*g
    v = b_2*v+(1-b_2)*(g**2)
    m_h = m/(1-(b_1**(i+1)))
    v_h = v/(1-(b_2**(i+1)))
...
def lower_bound(params,num_samples,num_particles):
    S = num_samples
    samples = sample_theta(params,S)
    return (log_variational(params,samples)-h_s(samples, num_particles))/S

def grad_KL(params, num_samples, num_particles,LB):
    S = num_samples
    samples = sample_theta(params,S)
...
def BBVI(K, param_init_range,num_samples,num_sims):
    params = np.random.uniform(0,param_init_range,2)
    m = np.array([0.,0.])
    v = np.array([0.,0.])
    lower_bounds = []
    iterating =1
    ...
    while i<2000:#iterating ==1:
        params,m,v,LB = iterate(params,num_samples,num_sims,i,m,v)
        #if params[1] <= 0:
        #    params = np.random.uniform(0,param_init_range,2)
        i+=1
...
if __name__=='__main__':
    ...
    param_init_range = 1
    for SP in ls_SP:
        params, i = BBVI(100,param_init_range,SP,SP)
        plt.plot(x,np.exp(log_variational(params,x)),label='BBVI/VBIL %i %i' % (SP,i))
